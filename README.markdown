...
The `debug` level is special because it's used almost exclusively during development and testing of features.  `debug` is most useful when combined with `tail -f` [1](http://en.wikipedia.org/wiki/Tail_(Unix)#File_monitoring)
